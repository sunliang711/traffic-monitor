...
func RegisterDatabaseLifecycle(lifecycle fx.Lifecycle, db *gorm.DB, cfg config.DatabaseConfig, log zerolog.Logger) {
	lifecycle.Append(fx.Hook{
		OnStart: func(startContext context.Context) error {
			...
			if err := db.WithContext(startContext).AutoMigrate(&model.Admin{}, &model.SSHKey{}, &model.Machine{}, &model.GlobalThresholdRule{}, &model.MachineThresholdRule{}, &model.TrafficSample{}, &model.Alert{}, &model.NotificationChannel{}, &model.NotificationProxy{}, &model.NotificationDelivery{}, &model.AppSetting{}); err != nil {
